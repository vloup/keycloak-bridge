...
func TestGetStatisticsAuthentications(t *testing.T) {
	...
	{ // fails - statistics by hours
		mockDBModule.EXPECT().GetTotalConnectionsHoursCount(ctx, realm).Return([][]int64{}, errors.New("error")).Times(1)
		res, err := component.GetStatisticsAuthentications(ctx, realm, "hours")
		assert.NotNil(t, err)
		assert.Nil(t, res)
	}
	{ // fails - statistics by days
		mockDBModule.EXPECT().GetTotalConnectionsDaysCount(ctx, realm).Return([][]int64{}, errors.New("error")).Times(1)
		res, err := component.GetStatisticsAuthentications(ctx, realm, "days")
		assert.NotNil(t, err)
		assert.Nil(t, res)
	}
	{ // fails - statistics by months
		mockDBModule.EXPECT().GetTotalConnectionsMonthsCount(ctx, realm).Return([][]int64{}, errors.New("error")).Times(1)
		res, err := component.GetStatisticsAuthentications(ctx, realm, "months")
		assert.NotNil(t, err)
		assert.Nil(t, res)
	}
	{ // fails - invalid param
		res, err := component.GetStatisticsAuthentications(ctx, realm, "ramdom")
		assert.NotNil(t, err)
		assert.Nil(t, res)
	}

	{ // success
		mockDBModule.EXPECT().GetTotalConnectionsDaysCount(ctx, realm).Return(statisticsKC, nil).Times(1)
		res, err := component.GetStatisticsAuthentications(ctx, realm, "days")
		assert.Nil(t, err)
		assert.Equal(t, statisticsKC, res)
	}
